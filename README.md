...
- 현재 역할: 로봇 애플리케이션 엔지니어
- 관심 분야: AGV/AMR/RobotArm, ROS, C++/Qt, Python, 스마트팩토리, 물류 자동화
...
**저의 AGV, AMR Middleware, 협동로봇 개발 경력과 현장경험을 토대로 직접 기획하고 AI Tool(ClaudeCode && Codex)을 활용**해 구현했습니다. 
...
이 프로젝트는 실제 장비 연동 업무를 위해 만들었습니다. 통신 포트나 소켓이 없으면 성공한 척하지 않고 실패하도록 만들고, 하드웨어가 없을 때는 가상 시리얼 페어, 로컬 에코, 디바이스 에뮬레이터로 검증할 수 있게 구성했습니다.
...
레포지토리 : 비공개 ( 추후 프로젝트 완성 후 일괄공개 예정 )

직접 AMR 부품구입, 프로그램 개발까지 진행하는 프로젝트입니다. 
주요 하드웨어는 NVIDIA JetsonOrin + STM 보드이며 , SLAM, 음성인식, 물체인식 기능까지 구현 예정입니다.
